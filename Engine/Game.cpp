...
{
    float m_DeltaTime = float(timer.GetElapsedSeconds());

	//note that currently.  Delta-time is not considered in the game object movement. 
	if (m_gameInputCommands.left)
	{
		//Vector3 rotation = m_Camera01.getRotation();
		//rotation.y = rotation.y += m_Camera01.getRotationSpeed()*m_DeltaTime;
		//m_Camera01.setRotation(rotation);
        Vector3 position = m_Camera01.getPosition(); //get the position
        position -= (m_Camera01.getRight() * m_Camera01.getMoveSpeed()) * m_DeltaTime;
	...
	{
		Vector3 position = m_Camera01.getPosition(); //get the position
		position += (m_Camera01.getForward()*m_Camera01.getMoveSpeed()* m_DeltaTime); //add the forward vector
		m_Camera01.setPosition(position);
	}
    ...
    if (m_gameInputCommands.down)
    {
        ...
        Vector3 up;
        up.y = 1;
        position -= (up * m_Camera01.getMoveSpeed() * m_DeltaTime);
        m_Camera01.setPosition(position);
    }
    ...
    Vector3 rotation = m_Camera01.getRotation();
    float mouseX = m_gameInputCommands.mouseDX;
    float mouseY = m_gameInputCommands.mouseDY;

    rotation.y = rotation.y += m_Camera01.getRotationSpeed() * mouseX * m_DeltaTime;
    rotation.x = rotation.x += m_Camera01.getRotationSpeed() * mouseY * m_DeltaTime;
...
{	
    ...
    m_sprites->Begin();
        ...
        m_font->DrawString(m_sprites.get(), L"Press C--down Space--up", XMFLOAT2(10, 40), Colors::Yellow);
    m_sprites->End();
